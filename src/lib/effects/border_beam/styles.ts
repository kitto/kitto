...
function getLineBloomGradients(colorVariant: BorderBeamColorVariant, isDark: boolean, id: string): string {
	...
	const spikes = bloomData.spikes.map(s =>
		isMono ? { color1: attenuateSpike(s.color1, att), color2: attenuateSpike(s.color2, att * 0.7) } : s
	)
...
function pulseTableGradients(table: PulseGradientDef[], variant: BorderBeamColorVariant, id: string): string {
	const palette = colorPalettes[variant].border
	return table
		.map(e => {
