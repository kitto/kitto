...
function swapBlockedBy(): string | null {
	...
	if ((window.devicePixelRatio || 1) !== spriteDpr)
		return 'display scale changed since the sprite was set — reload'
...
export function getGravityStatus(): {
	orbs: number
	tracking: boolean
	active: boolean
	...
	return {
		...
		tracking,
		active: curShown,
		blockedBy: instances.size === 0 ? 'no orb has gravity on' : blocked
	}
}
...
function ensureCursor(): boolean {
	...
	el.style.cssText =
		'position:fixed;left:0;top:0;pointer-events:none;z-index:2147483001;will-change:transform;display:none'
...
function bendSprite(B: number, taper: number, P: number, hx: number, hy: number, cxo: number, cyo: number): void {
	...
	const tux = tdx / tdl,
		tuy = tdy / tdl
	const x1 = Math.max(0, Math.floor(P + Math.min(0, B * tux) - 3)),
		x2 = Math.min(OW, Math.ceil(P + SW + Math.max(0, B * tux) + 3))
	const y1 = Math.max(0, Math.floor(P + Math.min(0, B * tuy) - 3)),
		y2 = Math.min(OH, Math.ceil(P + SH + Math.max(0, B * tuy) + 3))
	for (let y = y1; y < y2; y++) {
		for (let x = x1; x < x2; x++) {
			const i = (y * OW + x) * 4
			let sx = x - P,
...
function drawCursor(inst: Instance, w: number, dt: number): void {
	...
	if (dpr !== curDpr || sp.width !== curW || sp.height !== curH) {
		...
		curW = sp.width
		curH = sp.height
		const OW = Math.ceil((sp.width + 2 * PAD) * dpr),
			OH = Math.ceil((sp.height + 2 * PAD) * dpr)
	...
	const P = PAD * dpr
	const hx = P + sp.hotX * dpr,
		hy = P + sp.hotY * dpr
	const cxo = hx + (wcx - lpx) * dpr,
		cyo = hy + (wcy - lpy) * dpr
	const dTip = Math.hypot(cxo - hx, cyo - hy) || 1
	const ux = (cxo - hx) / dTip,
		uy = (cyo - hy) / dTip
	...
	if (o.squash > 0) {
		...
		const axx = (sp.width * 0.5 - sp.hotX) / axL,
			axy = (sp.height - sp.hotY) / axL
	...
	const bx1 = Math.floor(Math.min(P, P + ux * A) - grow),
		by1 = Math.floor(Math.min(P, P + uy * A) - grow)
	const bx2 = Math.ceil(Math.max(P, P + ux * A) + sp.width * dpr + grow),
		by2 = Math.ceil(Math.max(P, P + uy * A) + sp.height * dpr + grow)
	const rx = Math.max(0, bx1),
		ry = Math.max(0, by1)
	const rw = Math.min(OW, bx2) - rx,
		rh = Math.min(OH, by2) - ry
	...
	if (A > 0.5) {
		...
		const bcx = P + sp.width * dpr * 0.42,
			bcy = P + sp.height * dpr * 0.5
		const half = F / 2
...
function stepInner(now: number): void {
	...
	if (!Number.isNaN(px) && swapAllowed()) {
		...
		for (const inst of instances) {
			...
			if (px < cx - orbR - reach || px > cx + orbR + reach || py < cy - orbR - reach || py > cy + orbR + reach)
				continue
