 ...
 */
import { RANGE_PX } from './constants.js'
...
export function isHorizontalNeighbour(
	...
	const verticalOverlap =
		Math.min(anchorRect.bottom, targetRect.bottom) - Math.max(anchorRect.top, targetRect.top)
...
export function isVerticalNeighbour(
	...
	const horizontalOverlap =
		Math.min(anchorRect.right, targetRect.right) - Math.max(anchorRect.left, targetRect.left)
...
export function drawSource(
	...
	ctx.drawImage(src, dst.sx ?? 0, dst.sy ?? 0, sw, sh, dst.flipX ? 0 : dst.x, dst.flipY ? 0 : dst.y, dst.w, dst.h)
	ctx.restore()
}
