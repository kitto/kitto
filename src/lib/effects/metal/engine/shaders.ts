 ...
 *
 * Source-of-truth: Paper Shaders' `liquidMetal`, vendored unmodified from
 * `@paper-design/shaders@0.0.80` (see the vendored block below) so the
 * material stays byte-identical to what the Paper editor previews. (Upstream
 * metal-fx imports it from npm at build time; Kitto has no runtime deps and
 * svelte-package does not bundle, so it is inlined here.)
 *
...
/* ──────────────────────────────────────────────────────────────────────────
 * BEGIN VENDORED CODE — Paper Shaders (https://github.com/paper-design/shaders)
 * Copyright 2026 Paper. Licensed under the Apache License, Version 2.0
 * (http://www.apache.org/licenses/LICENSE-2.0). See ../NOTICE.md.
 *
 * Source package: @paper-design/shaders@0.0.80 (npm tarball)
 *   - `declarePI`, `rotation2`, `colorBandingFix`, `simplexNoise`:
 *       dist/shader-utils.js
 *   - `liquidMetalFragmentShader`:
 *       dist/shaders/liquid-metal.js
 *
 * UNMODIFIED: the GLSL template literals below are copied byte-for-byte from
 * the files above (only `export` was added to the fragment shader constant).
 * Kitto ships no runtime dependencies and svelte-package does not bundle, so
 * the shader is vendored here instead of imported.
 * ────────────────────────────────────────────────────────────────────────── */
const declarePI = `
...
`
/** Paper's liquidMetal fragment stage (@paper-design/shaders@0.0.80, Apache-2.0), unmodified. */
...
`
/* END VENDORED CODE — Paper Shaders */
