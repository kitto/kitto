import { acquireAnalyser } from './audio.js'
import { voiceLobes, LOBE_SPAN } from './styles.js'
...
function fitFilterRegion(inst: VoiceInstance, pts: Array<[number, number]>, ch: number): void {
	...
	const top = Math.max(
		0,
		Math.min(0.9, Math.floor((minY - FILTER_TOP_MARGIN) / ch / FILTER_TOP_STEP) * FILTER_TOP_STEP)
	)
...
function drawBand(inst: VoiceInstance, f: BandFrame, pts: Array<[number, number]>): void {
	...
	const colours = {
		r: config.bandColors.above,
		g: config.bandColors.mid,
		c: config.bandColors.core,
		b: config.bandColors.below
	}
...
function frame(ts: number): void {
	...
	instances.forEach(inst => {
		...
		el.style.setProperty(
			`--vb-cy-${config.id}`,
			`${(-cornerLift(beamAbsX, cw, arcRadius, lobeReach * 1.4) * cornerBlend).toFixed(1)}px`
		)
		...
		if (inst.displace && !warpOff) {
			const amount = config.reducedMotion
				? 0
				: config.distortion * 120 * config.scale * (0.15 + 0.85 * eff) * warp
		...
		for (let i = 0; i < voiceLobes.length; i++) {
			...
			el.style.setProperty(
				`--vb-y${i}-${config.id}`,
				`${(-cornerLift(lobeAbsX, cw, arcRadius, lobeReach) * cornerBlend).toFixed(1)}px`
			)
		}
