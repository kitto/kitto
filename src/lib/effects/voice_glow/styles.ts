import type { VoiceThemeColors, VoiceBeamColorVariant } from './types.js'
...
export function generateVoiceBeamCSS(options: GenerateVoiceStylesOptions): string {
	...
	const strokeGradients = lobeGradients({
		id,
		colors,
		alpha: 1,
		sw: gw(strokeScale),
	...
	const innerGradients = lobeGradients({
		id,
		colors,
		alpha: 0.46,
		sw: gw(0.9 * innerScale),
		sh: gh(0.9 * innerScale * innerHeight),
		y: 0,
		fade
	})
	const bloomGradients = lobeGradients({
		id,
		colors,
		alpha: isDark ? 0.9 : 0.7,
		sw: gw(1.15 * bloomScale),
		sh: gh(1.5 * bloomScale * bloomHeight),
		y: 0,
		fade: Math.min(95, fade + 2)
	})
