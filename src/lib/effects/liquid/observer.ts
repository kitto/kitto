import { measureRadius } from './geometry.js'
...
const viteHot = (import.meta as { hot?: { accept(cb: () => void): void; invalidate(): void } }).hot
...
function springStep(cur: number, vel: number, target: number, k: number, c: number, dt: number): [number, number] {
...
export class ObserveEngine {
	...
	private syncMelt(item: Item): void {
		...
		const t = item.target
		const imgs =
			t instanceof HTMLImageElement ? [t] : (Array.from(t.querySelectorAll('img')) as HTMLImageElement[])
	...
	private writeBlend(item: Item, dt: number): boolean {
		...
		if (bestOther && bestGap < range && blend.active !== false) {
			...
			const sunk = smoothstep(Math.max(0, Math.min(1, (embed - sink * 0.2) / Math.max(0.01, sink * 0.8))))
		...
		if (melt.seam) {
			...
			const fp =
				`${sBlur}|${sOp}|${q(bx, 0.5)},${q(by, 0.5)},${sR}|${zoneT}` + `|${regionX},${regionY},${regionW}`
			if (seam.last !== fp) {
				seam.last = fp
		...
		for (const entry of melt.entries) {
			...
			const hd = q(Math.min(Math.max(d, contactSpan * 0.75) * Math.min(kx, ky), rim), 1)
